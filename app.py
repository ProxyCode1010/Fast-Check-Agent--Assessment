...
def run_pipeline(uploaded_file, max_claims: int) -> None:
    with st.status("Reading PDF…", expanded=True) as status:
        ...
        try:
            ...
            status.update(label="Groq rate limit reached.", state="error")
            st.error(
                "Groq's free-tier limit for this model has been used up for now "
                f"({exc}).\n\n"
                "This usually clears within an hour. In the meantime you can switch "
                "to a higher-quota model by adding this to your Secrets (no code "
                "change needed):\n\n"
            ...
            status.update(label="Invalid Groq API key.", state="error")
            st.error(
                "Groq rejected the API key. Double-check `GROQ_API_KEY` in your "
                "Secrets -- it may be missing, truncated, or have a stray space."
            )
            return
        except GroqError as exc:
            status.update(label="Groq API error.", state="error")
            st.error(f"The claim-extraction step failed: {exc}")
            return
